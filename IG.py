...
class instagram:
	...
	def logo(self):
		for i in external:
			...
			self.mentod()
			urut=[]
			urut.append(Panel(f"{M2}Nama      : {H2}{nama}\n{P2}Username  : {U2}{self.username}\n{K2}Pengikut  : {P2}{followers}\n{O2}Mengikuti : {B2}{following}",title=f"{M2}• {K2}• {H2}•{P2} {P2}Pengguna {H2}• {K2}• {M2}•",width=40,padding=(0,1),style=f"{color_table}"))
			prints(Panel(f"{P2}\t      {B2}{nama}{B2} ᴀᴋᴜɴ ᴛᴜᴍʙᴀʟ",width=70,style=f"#FF00FF"))			          
			prints(Panel(f"""{M2}[{K2}{1}{M2}]{O2} ᴅᴀʀɪ ᴘᴇɴᴄᴀʀɪ ɴᴀᴍᴀ    {M2}[{K2}{4}{M2}]{M2} ᴄʀᴀɴᴄᴋ ʜᴀꜱɪʟ [yellow]CP
{M2}[{K2}{2}{M2}]{P2} ᴄʀᴀᴄᴋ ᴅᴀʀɪ ᴘᴇɴɢɪᴋᴜᴛ  {M2}[{K2}{5}{M2}]{H2} ʟɪʜᴀᴛ ʜᴀꜱɪʟ
{M2}[{K2}{3}{M2}]{K2}.ᴄʀᴀᴄᴋ ᴅᴀʀɪ ᴍᴇɴɢɪᴋᴜᴛɪ {M2}[{K2}{6}{M2}]{B2}. [red]LogOut """,title=f"[{B2} Pilihan Menu {hapus}]",width=70,padding=(0,4),style=f"#FFFFFF"))
	def BUG(self):
 ...
 {SE}╰─────────────────────────────────╯{SE} ╰───────────────────────────────────╯''',title=f"[[white] Cek Hasil [/]]",width=80,style=f"{color_table}"))
	...
	def passwordAPI(self,xnx):
		...
		if c in ["01","1"]:
			...
			prints(Panel(f"{P2}contoh password : sayang,anjing,bangsat",width=70,padding=(0,4),style=f"{color_table}"))
				...
				try:
					...
					if "logged_in_user" in str(ncek):
						...
						try:
							...
							print("")
							prints(Panel(f'\r{P2}[{H2}✓{M2}] 𝙉𝘼𝙈𝘼      : {H2}{nama}                \n{P2}[{H2}✓{B2}] 𝙐𝙎𝙀𝙍𝙉𝘼𝙈𝙀  : {K2}{user}                \n{P2}[{H2}✓{K2}] 𝙋𝘼𝙎𝙎𝙒𝙊𝙍𝘿  : {B2}{pw}                \n{P2}[{H2}✓{O2}] 𝙁𝙊𝙇𝙊𝙒𝙀𝙍   : {N2}{pengikut}                \n{P2}[{H2}✓{P2}] 𝙁𝙊𝙇𝙊𝙒𝙄𝙉𝙂  : {M2}{mengikut}                \n{P2}[{H2}✓{H2}] 𝙋𝙊𝙎𝙏𝙄𝙉𝙂𝘼𝙉 : {O2}{postingan}\n{P2}[{H2}✓{K2}] 𝙉𝙊𝙈𝙊𝙍     : {B2}{nomo}\n{P2}[{H2}✓{M2}] 𝙀𝙈𝘼𝙄𝙇     : {K2}{email}\n{P2}[{H2}✓{P2}] 𝘾𝙊𝙆𝙄𝙀𝙎    : {P2}{cookie}',title=f"{H2} 𝙉𝙂𝘼𝘾𝙀𝙉𝙂",width=70,padding=(0,4),style="#00C8FF"))
							print("")
	...
	def menu(self):
		self.logo()
		prints(Panel(f"""\t {P2}Ketik {M2}Bug {P2}Untuk Melaporkan Bug Script""",width=70,padding=(0,7),style=f"{color_table}"))
		c=input(f'•{N} Pilih :{C}  ')
		if c=='':
			self.menu()
		elif c in ('1','01'):
			prints(Panel(f"{P2}Crack Dari Pencarian Nama",width=70,padding=(0,2),style=f"{color_table}"))
			...
			prints(Panel(f"{P2}Crack Dari Pengikut",width=70,padding=(0,2),style=f"{color_table}"))
			#massal(self)
			...
			prints(Panel(f"{P2}Crack Dari Mengikut",width=70,padding=(0,2),style=f"{color_table}"))
			mas=input('• Apakah anda ingin crack masal? y/t >  ')
			if mas in ['y','Y']:
				mengi(self)
			elif mas in ['t','T']:
...
def mengi(self):
			try:
				...
				prints(Panel(f"{P2}Pastikan Target Bersifat Publik {M2}!!!!",width=70,padding=(0,2),style=f"{color_table}"))
				m=int(input(f'{N}•{N} Masukan jumlah target: {N}'))
			except:m=1
			for t in range(m):
				t +=1
				print('\r')
				prints(Panel(f"{P2}Total Username Terkumpul : {H2}{len(internal)}",width=70,padding=(0,20),style=f"{color_table}"))
				nama=input(f' [{t}] Masukan Username : ')
				prints(Panel(f"[white]Tekan [red]CTRL + C[white] Untuk Berhenti",width=70,padding=(0,4),style=f"{color_table}"))
